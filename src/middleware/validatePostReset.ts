...
/**
 * Validate reset-account requests and ensure phone matches the hashed token user id.
 */
export const validatePostReset = async (req: Request, res: Response, next: NextFunction) => {
    const { cid, phone } = req.body;
    if (!cid || !phone) return ResponseHandler.error(req, res, ErrorCodes.missingRequiredData);
